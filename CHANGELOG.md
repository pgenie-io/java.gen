...
- Bumped `gen-sdk` to v2.0.0, following the `Sdk.Sigs` restructuring: `Sigs.Interpreter.module`/`Sigs.Template.module` became lowercase `Sigs.interpreter`/`Sigs.template`, and the top-level entry point now builds its `Generator` via the new `Sdk.Sigs.generator Config defaultConfig interpret` in place of the removed `Contract.module`. No change to generated output.
